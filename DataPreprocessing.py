...
wordVocab = {}
tag_mapping = {}
count_until = Pre.parse_Questions(treebank,0)
Pre.parse_Questions(brown,count_until)
Pre.labelWordEncoder()
brown_tagsents = dllist(treebank.tagged_sents(tagset='universal'))
xTrain, yTrain = Pre.dataSegmentation(brown_tagsents)

############################################              MODEL                    ########################################################################
# Data Partition
# Padding Vectorized Data Set
x_train = tf.keras.preprocessing.sequence.pad_sequences(xTrain[:3914], padding='post')
y_train = tf.keras.preprocessing.sequence.pad_sequences(yTrain[:3914], padding='post')

x_validation = x_train[3000:]
y_validation = y_train[3000:]
...
model.add(layers.Embedding(input_dim=vocab_size, input_length=271, output_dim=200, mask_zero=True))
